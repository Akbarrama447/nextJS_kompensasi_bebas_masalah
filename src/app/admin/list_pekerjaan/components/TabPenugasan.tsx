        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-4">
            ...
            <div className="flex gap-3 mb-4">
                ...
                <div className="flex border border-gray-200 rounded-lg overflow-hidden divide-x divide-gray-200 shadow-sm">
                    <button className="px-4 py-2 bg-blue-50 text-blue-600 font-medium text-sm hover:bg-blue-100 transition-colors focus:outline-none">
                        Pending
                    </button>
                    <button className="px-4 py-2 text-sm text-gray-600 bg-white hover:bg-gray-50 transition-colors focus:outline-none">
                        Selesai
                    </button>
                    <button className="px-4 py-2 text-sm text-gray-600 bg-white hover:bg-gray-50 transition-colors focus:outline-none">
                        Semua
                        ...
                        {dummyData.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                                <td className="px-4 py-3">
                                    ...
                                    <p className="text-xs text-blue-600 mt-0.5">{item.nim} {item.kelas}</p>
                                </td>
                                <td className="px-4 py-3 text-gray-600">{item.pekerjaan}</td>
                                <td className="px-4 py-3 text-center text-gray-600">{item.jam}</td>
                                <td className="px-4 py-3 text-center text-gray-600">{item.tanggal}</td>
                                <td className="px-4 py-3 text-center">
                                    <span className="px-2.5 py-1 bg-yellow-50 text-yellow-600 border border-yellow-200 rounded-md text-xs font-medium">{item.status}</span>
                                </td>
                                <td className="px-4 py-3 text-center">
                                    <button 
                                        onClick={() => openVerifikasi(item)}
                                        className="text-[var(--color-primary)] hover:text-white bg-blue-50 hover:bg-[var(--color-primary)] px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border border-blue-100 hover:border-[var(--color-primary)] focus:outline-none"
                                    >
                                        Verifikasi
                                    </button>
                                </td>
