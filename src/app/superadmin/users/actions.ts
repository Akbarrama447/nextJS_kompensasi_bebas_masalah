...
export async function createUser(data: {
  ...
  try {
    ...
    // Identifikasi tipe entitas dari role name
    const isMahasiswaRole = roleName === 'mahasiswa'
    ...
    if (isMahasiswaRole) {
      ...
      // Semua role selain Mahasiswa adalah Staf (Super Admin, Staf Jurusan, Dosen, dll)
      if (!nimOrNip) {
        return { success: false, error: 'NIP wajib diisi untuk role Staf/Admin/Dosen!' }
      }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (isMahasiswaRole) {
        ...
        // Super Admin, Staf Jurusan, Dosen, dll — semuanya masuk tabel staf
        const finalNip = nimOrNip || `SA_${Date.now()}`
        
        // Tentukan tipe_staf berdasarkan role
        let tipeStaf = 'admin'
        if (roleName === 'super admin' || roleName === 'superadmin') {
          tipeStaf = 'superadmin'
        } else if (roleName === 'dosen') {
          tipeStaf = 'dosen'
        } else if (roleName === 'staf jurusan' || roleName === 'staf') {
          tipeStaf = 'staf'
        }
...
export async function updateUser(data: {
  ...
  try {
    ...
    // Helper untuk menentukan tipe_staf dari role name
    const getTipeStaf = (rName: string) => {
      if (rName === 'super admin' || rName === 'superadmin') return 'superadmin'
      if (rName === 'dosen') return 'dosen'
      if (rName === 'staf jurusan' || rName === 'staf') return 'staf'
      return 'admin'
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!wasMahasiswa && isMahasiswaRole) {
      ...
      } else if (wasMahasiswa && !isMahasiswaRole) {
        ...
        await tx.staf.create({
          data: {
            ...
            user_id: userId,
            jurisdiction_id: jurusanId || null,
            tipe_staf: getTipeStaf(roleName)
          }
        })
      } else if (wasMahasiswa && isMahasiswaRole) {
